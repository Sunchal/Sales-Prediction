...
/*This program creates an array of Department structures to store the data for each department,
including the name, current stock level, predicted sales, and ordered items.
It prompts the user to enter the predicted sales data for each department and stores it in the corresponding structure.
It then uses an appropriate forecasting method to predict future sales for each department. 
Finally, it checks the current stock levels for each department and orders more items if necessary.
It then displays the sales data and stock levels for each department in a table format.*/